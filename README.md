...
<div align="center">
  <strong>A Swi blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/swincacoin/insight-api">
    <img src="https://img.shields.io/travis/swincacoin/insight-api/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/@swincacoin/insight-api">
    <img src="https://img.shields.io/npm/v/@swincacoin/insight-api.svg?style=flat-square" alt="NPM version" />
  </a>
</div>


This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/swincacoin/insight-ui.
...
- [Resources](#resources)
- [License](https://github.com/swincacoin/insight-api-swi/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g swicore-node@latest
bitcore-node-swi create mynode
cd mynode
swicore-node install insight-api
swicore-node start
```
...
- [Bitcore Node Swi 3.x](https://github.com/swincacoin/swicore-node)

**Note:** You can use an existing Swi data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `swi.conf`, as well as a few other additional fields.
...
```
With all the configuration options available: https://github.com/swincacoin/insight-api/blob/master/lib/ratelimiter.js#L10-17
...
### Block
```
  /insight-api-swi/block/[:hash]
  /insight-api-swi/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-swi/block-index/[:height]
  /insight-api-swi/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-swi/rawblock/[:blockHash]
```
...
```
  /insight-api-swi/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-swi/tx/[:txid]
  /insight-api-swi/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-swi/rawtx/[:rawid]
  /insight-api-swi/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-api-swi/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-api-swi/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-api-swi/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-api-swi/addr/[:addr]/balance
  /insight-api-swi/addr/[:addr]/totalReceived
  /insight-api-swi/addr/[:addr]/totalSent
  /insight-api-swi/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-api-swi/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-api-swi/addrs/[:addrs]/utxo
  /insight-api-swi/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-api-swi/addrs/utxo
```
...
```
  /insight-api-swi/txs/?block=HASH
  /insight-api-swi/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-api-swi/txs/?address=ADDR
  /insight-api-swi/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-api-swi/addrs/[:addrs]/txs[?from=&to=]
  /insight-api-swi/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-api-swi/addrs/txs
```
...
GET method:
```
  /insight-api-swi/gobject/list/proposal (or /insight-api-swi/gobject/list)
```
...
GET method:
```
  /insight-api-swi/gobject/list/trigger
```
...
GET method:
```
  /insight-api-swi/gobject/get/[:hash]
  /insight-api-swi/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
GET method:
```
  /insight-api-swi/gobject/check/[:hexData]
  /insight-api-swi/gobject/check/5b5b2270726f706f736[..]
```
...
GET method:
```
  /insight-api-swi/gobject/deserialize/[:hexData]
  /insight-api-swi/gobject/deserialize/5b5b2270726f706f736[..]
```

Sample output:
```
{
  "result":"[[\"proposal\",{\"end_epoch\":1519848619,\"name\":\"ghijklmnopqrstuvwxyz01234567891519097947\",\"payment_address\":\"yik5HAgVAgjH1oZKjcDfvcf22bwBNbSYzB\",\"payment_amount\":10,\"start_epoch\":1519097947,\"type\":1,\"url\":\"https://www.swicentral.org/p/test_proposal_1519097947\"}]]",
  "error":null,
  "id":78637
}
```
...
GET method:
```
  /insight-api-swi/gobject/votes/current/[:hash]
  /insight-api-swi/gobject/votes/current/fbda8cdc1f48917f53b7d63fbce81c85d6dedd3d0e476e979926dfd154b84034
```
...
GET method:
```
  /insight-api-swi/governance/budget/[:blockIndex]
  /insight-api-swi/governance/budget/79872
```
...
POST method:
```
  /insight-api-swi/gobject/submit
```
...
### Masternodes List
```
  /insight-api-swi/masternodes/list
```
### Validate Masternode
```
  /insight-api-swi/masternodes/validate/[:payee]
  /insight-api-swi/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-api-swi/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-swi/peer
```

### Status of the Bitcoin Network
```
  /insight-api-swi/status?q=xxx
```
...
### Utility Methods
```
  /insight-api-swi/utils/estimatefee[?nbBlocks=2]
```
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in swid.
- The endpoint for `/peer` is no longer relevant connection to swid is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking swid.
...
- (Medium)[How to setup a Swi Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
